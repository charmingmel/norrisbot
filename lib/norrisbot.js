...
NorrisBot.prototype._replyWithRandomJoke = function (originalMessage) {
    ...
    self.db.get('SELECT id, joke FROM jokes ORDER BY used ASC, RANDOM() LIMIT 1', function (err, record) {
        ...
        var currentUser = self._getUserById(originalMessage.user);

        if(currentUser) {
            self.postMessageToUser(currentUser.name, record.joke, {as_user: true});
        }
        else {
...
NorrisBot.prototype._isChannelConversation = function (message) {
    return typeof message.channel === 'string' && (message.channel[0] === 'C' || 'D');
};
